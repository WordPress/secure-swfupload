/*
  ...
  Summary:
    This plugin uses SWFObject to embed SWFUpload dynamically in the page.  SWFObject provides standards
    compliant flash markup, advanced Flash detection and Flash Express Install.

	Features:
		* swfupload_load_failed_hander event
		* express_install_handler event
		* enable_express_install setting (default: false)
		* minimum_flash_version setting (default: "9.0.28")

  Usage:
    Provide handlers and settings as needed.  SWFObject allows SWFUpload to be initialized directly in the page
    rather than in window.onload.  When initialized this way the movie can load earlier allowing SWFUpload to
    be ready when the page has finished loading.

    Explicitly returning true from the express_install_handler function will cause SWFUpload.SWFObject to display
    the Flash Express Install. If you wish to handle express install manually return some other value (or do not
    return).  If you do not provide an express_install event handler then SWFUpload.SWFObject will behave as
    if you returned true from a handler automatically displaying the Express Install.

    <script type="text/javascript">
      var swfu = new SWFUpload({
        // swfupload settings
        minimum_flash_version: "9.0.28",
        enable_express_install: true
        express_install_handler: expressInstall,
...
if (typeof(SWFUpload) === "function") {
	...
	SWFUpload.prototype.initSettings = (function (oldInitSettings) {
		return function () {
			...
			this.ensureDefault("enable_express_install", false);
			this.ensureDefault("swfupload_load_failed_handler", null);
			this.ensureDefault("express_install_handler", null);
	...
	SWFUpload.prototype.loadFlash = function () {
		// Check Version, call swfupload_load_failed if we don't have the right version
		// we should only do express install on windows
		
		// use SWFObject to load flash here
	};
	
	delete SWFUpload.prototype.getFlashHTML;

	SWFUpload.prototype.getFlashVars = function () {
		// Get SWFObject flash vars
	};
		...
		return function () {
			...
			this.debug(
				[
					...
					"\t", "enable_express_install:            ", this.settings.enable_express_install.toString(), "\n",
					"\t", "swfupload_load_failed_handler:     ", this.settings.swfupload_load_failed_handler.toString(),, "\n",
					"\t", "express_install_handler:           ", this.settings.express_install_handler.toString(),, "\n",
				].join("");
			);
		};	
